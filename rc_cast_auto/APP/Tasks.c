...
void MoveTask(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        do
        {
            if (!Arrived && DeltaPos(GoalX - PosX, GoalY - PosY) <= 0.025 && fabs(GoalZ - PosZ) <= 1.00 && DeltaPos(VelX, VelY) <= 0.05)
            {
        ...
        for (uint8_t i = 0; i < 5; i++) {Buzzer_On(); delay_ms(250); Buzzer_Off(); delay_ms(750);}
    }
}
