...
class VL53L0X:
    ...
    def readData(self, Reg, num):
        if currentBoard == "esp8266" or currentBoard == "esp32":
            self.originalData = self.i2c.readfrom_mem(self.I2C_DevAddr, Reg,
                                                      num)
        elif currentBoard == "pyboard":
    ...
    def readVL53L0X(self):
        ...
        detailedData['ambientCount'] = (
            (self.originalData[6] & 0xFF) << 8) | (self.originalData[7] & 0xFF)
        detailedData['signalCount'] = (
            (self.originalData[8] & 0xFF) << 8) | (self.originalData[9] & 0xFF)
        detailedData['distance'] = ((self.originalData[10] & 0xFF) << 8) | (
            self.originalData[11] & 0xFF)
    ...
    def start(self):
        ...
        for x in [(0x80, 1), (0xff, 1), (0, 0), (0x91, 0x3c), (0, 1),
                  (0xff, 0), (0x80, 0)]:
            self.writeData(x)
        if DeviceMode == VL53L0X_DEVICEMODE_SINGLE_RANGING:
            ...
            while True:
                ...
                LoopNb = LoopNb + 1
                if not (((Byte & StartStopByte) == StartStopByte) and
                        (LoopNb < VL53L0X_DEFAULT_MAX_LOOP)):
                    break
        elif DeviceMode == VL53L0X_DEVICEMODE_CONTINUOUS_RANGING:
            self.writeData((VL53L0X_REG_SYSRANGE_START,
    ...
    def dataInit(self):
        if ESD_2V8 == 1:
            ...
            data = (data & 0xFE) | 0x01
            self.writeData((VL53L0X_REG_VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV,
                            data))
            for x in [(0x88, 0), (0x80, 1), (0xff, 1), (0, 0), (0x91, 0x3c),
                      (0, 1), (0xff, 0), (0x80, 0)]:
                self.writeData(x)
